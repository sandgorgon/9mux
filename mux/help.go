...
var keybindingHelp = []string{
	...
	"  n / p          jump focus into the next / previous pane (also",
	"                 Right/Down and Left/Up; wraps around)",
	...
	"  A title bar with keyboard focus itself is drawn in its own color",
	"  (purple by default): keys there are commands, not typed into the",
	"  shell. Either way, a focused pane's title also shows a ● marker",
	"  (right before its [N] label) as a color-independent cue.",
	...
	"moves to this pane's own title bar, where the keys above take over",
	"-- so moving between shell panes is Ctrl+\\ then n/p/1-9.",
	"",
